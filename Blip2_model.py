...
class Blip2QFormerEncoder(nn.Module):
    ...
    def gradually_mask_attention(self, a, layer_index, total_layers, batch_object_patch_indices, total_patches=256):
        if not isinstance(a, torch.Tensor):
            raise ValueError("Input 'a' must be a tensor.")

        batch_size = a.size(0)
        ignore_value = torch.finfo(a.dtype).min  # Large negative value used for masking

        if layer_index == 0:
            return

        # For layers beyond the first, calculate the dynamic number of patches to mask based on layer
        proportion_to_mask = layer_index / (total_layers - 1)

        for batch_idx, object_patch_indices in enumerate(batch_object_patch_indices):
            all_patches = set(range(1, total_patches + 1))  # Patches indexed from 1 to 256
            significant_patches = set(object_patch_indices)
            insignificant_patches = all_patches - significant_patches

            # Determine the number of patches to mask in this layer
            num_patches_to_mask = int((total_patches - len(object_patch_indices)) * proportion_to_mask)
            num_patches_to_mask = min(len(insignificant_patches), num_patches_to_mask)

            if num_patches_to_mask > 0:
                patches_to_mask = set(random.sample(insignificant_patches, num_patches_to_mask))

                # Apply mask adjustments
                for patch_idx in patches_to_mask:
                    if patch_idx < a.size(-1):
                        a[batch_idx, 0, 0, patch_idx] = ignore_value
    ...
    def forward(
            ...
            return_dict=True,
            query_length=0,
            batch_object_patch_indices=None,
    ):
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if batch_object_patch_indices != None:
                self.gradually_mask_attention(encoder_attention_mask, i, self.config.num_hidden_layers,
                                              batch_object_patch_indices)
...
class Blip2QFormerModel(Blip2PreTrainedModel):
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(
            ...
            batch_object_patch_indices=batch_object_patch_indices,
        )
